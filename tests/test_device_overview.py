...
def test_device_api_combines_customer_rmm_glpi_and_checkmk():
    with _authenticated_client() as client:
        customer = client.post("/api/v1/customers", json={"customer_number": "MON-300", "name": "Monitoring Kunde"})
        assert customer.status_code in {201, 409}
        imported = client.post("/api/v1/devices/import", json={"source": "netlock", "external_id": "monitor-device-300", "customer_number": "MON-300", "hostname": "monitor-pc-300", "online_status": "online", "agent_status": "ok"})
        assert imported.status_code == 201
        device_id = imported.json()["device"]["id"]
        linked = client.patch(f"/api/v1/devices/{device_id}/glpi", json={"glpi_asset_id": "Computer:300"})
        assert linked.status_code == 200

    with _connection() as connection:
        host_id, _ = upsert_host(connection, {"id": "monitor-pc-300", "host_name": "monitor-pc-300", "state": 0, "last_check": "2026-09-14T18:00:00Z"}, site="prod")
    ...
    with _authenticated_client() as client:
        ...
        assert {item["description"] for item in detail.json()["monitoring_services"]} == {"CPU", "Disk"}


def test_device_monitoring_filter_supports_unmonitored():
    with _authenticated_client() as client:
        imported = client.post("/api/v1/devices/import", json={"source": "netlock", "external_id": "unmonitored-301", "hostname": "unmonitored-pc-301", "online_status": "unknown"})
        assert imported.status_code == 201
