...
def _incident(incident_key: str):
    with connect() as connection:
        return connection.execute("SELECT * FROM monitoring_incidents WHERE incident_key=?", (incident_key,)).fetchone()


async def process_monitoring_actions_once(limit: int = 10) -> int:
    ...
    for event in reversed(events):
        ...
        try:
            ...
            _save_action(event["id"], status=status, attempts=attempts, next_attempt_at=None if status == "failed" else retry_at(attempts), last_error=str(exc)[:2000])
            record_error("monitoring_action", event["id"], str(exc))
    return processed
